...
% ─── Title ───────────────────────────────────────────────────────────────
\title{%
  \textbf{QuantHive: Learned Adaptive Governance \\
...
We propose that institutional governance should not be a static script, but rather an \emph{adaptive, learned policy} capable of dynamically restricting or expanding operational bounds based on real-time market microstructure and portfolio drawdown. We introduce \textbf{QuantHive}, an open-source multi-agent framework built on PettingZoo \citep{terry2021pettingzoo}, where governance is modeled as a Decentralized Partially Observable Markov Decision Process (Dec-POMDP) \citep{oliehoek2016concise}.

In QuantHive, three distinct deep reinforcement learning agents interact sequentially using Proximal Policy Optimization (PPO) \citep{schulman2017proximal}: a Trader, a Risk Manager (RM), and a Portfolio Manager (PM). The RM and PM act as neural supervisors, imposing hierarchical boundary constraints on the Trader's action space. This creates an adversarial, mixed-motive environment where the Trader seeks to maximize absolute profit, while the supervisory agents seek to minimize tail-loss and ensure institutional survival.
...
Ensuring that autonomous systems operate within safe bounds is a core challenge in AI alignment \citep{amodei2016concrete}. While most safety research focuses on reward shaping or constrained MDPs, QuantHive approaches safety via \emph{learned oversight}---training an independent neural network to monitor and constrain a primary actor. This mirrors real-world institutional structures where Risk Departments operate independently of Trading Desks.

\textbf{Financial Simulation Environments.}
Prior work on RL for trading---including FinRL, ABIDES, and OpenAI's market makers---focuses primarily on profit maximization. QuantHive is distinguished by its explicit focus on \emph{governance quality} as the primary evaluation metric, rather than PnL.
...
QuantHive generates synthetic market data using a Markov-chain regime transition model with 8 standard regimes and 4 adversarial stress scenarios, each calibrated to real historical market parameters (Table~\ref{tab:regimes}).
